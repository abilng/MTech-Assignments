...
vector<bool> graph_property (bool** adj, int& number_of_edges, int n) {
                ...
                for (i = 0; i < n; i++)
                {
                    if (visited [i] == false)
                    {
                        ...
                        do
                        {
                            ...
                            next_index++;
                            i = spanning_tree [next_index]; // I HAD MISSED THIS STATEMENT *******************
...
void getEdges(bool** adj, int n, double p, int& number_of_edges) {
    cout << "Getting edges with p: " << p << endl;
    cout << "n is " << n << endl;
    for (int i = 0; i < n; i++) {
        adj[i][i] = false;
        for (int j = i + 1; j < n; j++) {
            double ep = ((double) rand() / (RAND_MAX+1));
            //cout << "HERE"<< ep << endl;
            if (ep <= p) {
                adj[i][j] = true;
                adj[j][i] = true;
...
double getProbability (int n, int fun) {
    double p;
    switch(fun) {
        case 0: { // constant probability of 0.5
            p = 0.075;
            return p;
        }
        case 1: {
